...
(function($) {
	...
	function _9(_a) {
		...
		_c.find(".calendar-menu-year").unbind(".calendar").bind(
		...
		function _10(_18) {
			...
			_c.find("td:eq(" + (_b.month - 1) + ")").addClass(
					"calendar-selected");
		}
		;
	}
	...
	function _5(_1a) {
		...
		if ($(_1a).find(".calendar-menu-month-inner").is(":empty")) {
			...
			var t = $("<table class=\"calendar-mtable\"></table>").appendTo(
					$(_1a).find(".calendar-menu-month-inner"));
			var idx = 0;
			for ( var i = 0; i < 3; i++) {
				var tr = $("<tr></tr>").appendTo(t);
				for ( var j = 0; j < 4; j++) {
					$("<td class=\"calendar-nav calendar-menu-month\"></td>")
							.html(_1b.months[idx++]).attr("abbr", idx)
							.appendTo(tr);
				}
			}
		}
	...
	function _19(_2f) {
		...
		var now = new Date();
		var _31 = now.getFullYear() + "," + (now.getMonth() + 1) + ","
				+ now.getDate();
		var _32 = _30.current ? (_30.current.getFullYear() + ","
				+ (_30.current.getMonth() + 1) + "," + _30.current.getDate())
				: "";
	...
	$.fn.calendar = function(_3c, _3d) {
		...
		return this.each(function() {
			...
			if (_3e) {
				...
				_3e = $.data(this, "calendar", {
					options : $.extend({}, $.fn.calendar.defaults,
							$.fn.calendar.parseOptions(this), _3c)
				});
				_6(this);
			}
